...
import { ArrowRight, BookOpen, Code, Users, Zap, ExternalLink, Github, MessageCircle, Rocket, Layers, Shield, ChevronDown, Star, Terminal, FileCode, Sparkles, CheckCircle, Circle, GraduationCap, Hammer, Send, Building, HelpCircle, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute w-96 h-96 rounded-full blur-3xl opacity-40" style={{ top: '5%', left: '5%', background: 'radial-gradient(circle, #FF8E02 0%, transparent 70%)', transform: `translate(${scrollY * 0.05}px, ${scrollY * 0.02}px)` }} />
        <div className="absolute w-80 h-80 rounded-full blur-3xl opacity-40" style={{ top: '40%', right: '5%', background: 'radial-gradient(circle, #FA2E04 0%, transparent 70%)', transform: `translate(${-scrollY * 0.03}px, ${scrollY * 0.04}px)` }} />
        <div className="absolute w-72 h-72 rounded-full blur-3xl opacity-30" style={{ bottom: '10%', left: '25%', background: 'radial-gradient(circle, #B70001 0%, transparent 70%)', transform: `translate(${scrollY * 0.02}px, ${-scrollY * 0.03}px)` }} />
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-black/70 border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-black">
              <KairoLogo className="w-6 h-6" />
            </div>
            <span className="font-bold text-xl">Canton<span className="text-orange-500">Hub</span></span>
          </div>
      ...
      <section className="relative min-h-screen flex items-center justify-center px-6 pt-20">
        <div className="max-w-5xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/10 border border-orange-500/20 mb-8">
            ...
            <span className="text-sm text-orange-300">Your Gateway to Canton Network</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="https://earn.stackup.dev/learn/pathways/canton-and-daml-fundamentals-328a" target="_blank" rel="noopener noreferrer" className="group px-8 py-4 rounded-xl bg-gradient-to-r from-orange-500 to-red-600 font-medium text-lg flex items-center gap-3 hover:shadow-lg hover:shadow-orange-500/25 transition-all text-black">
              Learn About Canton Network
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </a>
            <a href="#roadmap" className="px-8 py-4 rounded-xl bg-white/5 border border-white/10 font-medium text-lg flex items-center gap-3 hover:bg-white/10 hover:border-orange-500/30 transition-all">
              <Code className="w-5 h-5" />
              Start Building on Canton Network
            </a>
          </div>

          <div className="mt-16 flex items-center justify-center gap-8 text-sm text-gray-500">
            <div className="flex items-center gap-2"><Shield className="w-4 h-4 text-orange-500/70" /><span>Privacy-First</span></div>
            <div className="flex items-center gap-2"><Zap className="w-4 h-4 text-orange-500/70" /><span>Atomic Finality</span></div>
            <div className="flex items-center gap-2"><Layers className="w-4 h-4 text-orange-500/70" /><span>Interoperable</span></div>
          </div>
        </div>
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce">
          <ChevronDown className="w-6 h-6 text-orange-500/50" />
        </div>
      </section>
      ...
      <section id="roadmap" className="relative py-32 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold mb-4">Roadmap to Building on Canton</h2>
            <p className="text-gray-400 text-lg">Your three-step journey from learning to launch</p>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6 mb-20">
            {roadmapSteps.map((step, i) => (
              <div key={step.step} className="relative">
                ...
                <div className="h-full p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-orange-500/30 transition-all">
                  <div className="flex items-center gap-4 mb-6">
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${step.color} flex items-center justify-center text-black font-bold text-lg`}>
                      {step.step}
                    </div>
                    <div>
                  ...
                  {step.items && (
                    <div className="space-y-2">
                      {step.items.map((item, j) => (
                        <a key={j} href={item.link} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-colors ${item.type === 'recommended' ? 'bg-orange-500/10 border border-orange-500/20' : ''}`}>
                          <ExternalLink className="w-4 h-4 text-orange-500 flex-shrink-0" />
                          <span className="text-sm text-gray-300">{item.label}</span>
                          {item.type === 'recommended' && <span className="ml-auto text-xs bg-orange-500 text-black px-2 py-0.5 rounded-full">Recommended</span>}
                        </a>
                      ))}
                    </div>
                  )}
                  ...
                  {step.checklist && (
                    <div className="space-y-3 mb-4">
                      <p className="text-xs text-gray-500 uppercase tracking-wide">Development Checklist</p>
                      {step.checklist.map((item, j) => (
                        <a key={j} href={item.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-gray-300 hover:text-white transition-colors">
                          <Circle className="w-4 h-4 text-orange-500/50" />
                          {item.task}
                        </a>
                      ))}
                    </div>
                  ...
                  {step.resources && (
                    <div className="mt-4 pt-4 border-t border-white/10 space-y-2">
                      ...
                      {step.resources.map((res, j) => (
                        <a key={j} href={res.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-orange-400 hover:text-orange-300 transition-colors">
                  ...
                  {step.description && (
                    <div>
                      ...
                      <a href={step.cta.link} className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r ${step.color} text-black font-medium text-sm hover:shadow-lg transition-all`}>
                        {step.cta.label}
          ...
          <div className="mb-16">
            <h3 className="text-2xl font-bold mb-6 text-center">Canton vs EVM at a Glance</h3>
            <div className="bg-white/5 rounded-xl border border-white/10 overflow-hidden max-w-3xl mx-auto">
              <div className="grid grid-cols-3 gap-4 p-3 bg-white/5 text-sm font-medium">
              ...
              {faqs.map((faq, i) => (
                <div key={i} className="bg-white/5 rounded-xl border border-white/10 overflow-hidden">
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between p-4 text-left hover:bg-white/5 transition-colors">
                    ...
                    <ChevronRight className={`w-5 h-5 text-orange-500 transition-transform ${openFaq === i ? 'rotate-90' : ''}`} />
                  </button>
                  {openFaq === i && (
                    <div className="px-4 pb-4">
                      <p className="text-gray-400 text-sm">{faq.a}</p>
                      {faq.link && <a href={faq.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-orange-500 text-sm mt-2 hover:underline">Learn more <ExternalLink className="w-3 h-3" /></a>}
                    </div>
                  )}
                </div>
              ))}
      ...
      {/* Page 3: Partners */}
      <section id="partners" className="relative py-32 px-6">
        ...
        <div className="max-w-6xl mx-auto relative">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/10 border border-orange-500/20 mb-6">
              ...
              <span className="text-sm text-orange-300">Ecosystem Partners</span>
            </div>
            <h2 className="text-4xl font-bold mb-4">Featured Partners</h2>
            <p className="text-gray-400 text-lg max-w-2xl mx-auto">Building the Canton Network ecosystem together</p>
          </div>

          {/* Partner Grid */}
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
            {partners.map((partner, i) => (
              <div key={i} className="group p-6 rounded-xl bg-white/5 border border-white/10 hover:border-orange-500/30 transition-all text-center">
                <div className="w-16 h-16 mx-auto mb-4 rounded-xl bg-white/10 flex items-center justify-center text-gray-400 group-hover:bg-orange-500/20 group-hover:text-orange-500 transition-all">
          ...
          {/* Services */}
          <div className="mb-16">
            <h3 className="text-2xl font-bold mb-8 text-center">How We Can Help</h3>
            <div className="grid md:grid-cols-3 gap-6">
              {services.map((service, i) => (
                <div key={i} className="p-6 rounded-xl bg-gradient-to-br from-orange-500/10 to-red-500/10 border border-orange-500/20">
                  <h4 className="font-bold text-lg mb-2 text-orange-400">{service.title}</h4>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative p-12 rounded-3xl overflow-hidden">
            ...
            <div className="relative text-center">
              <h2 className="text-4xl font-bold mb-4 text-black">Ready to Build on Canton?</h2>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <a href="mailto:canton@angelhack.com" className="px-8 py-4 rounded-xl bg-black text-white font-semibold text-lg hover:bg-gray-900 transition-colors flex items-center gap-3">
                  <MessageCircle className="w-5 h-5" />
                  Talk to Our Team
                </a>
                <a href="https://docs.google.com/forms/d/e/1FAIpQLSeOigsGa_RLwLoGNZivrv1vZQkulVjueRyPYNStGVDRMUp-4A/viewform?pli=1" target="_blank" rel="noopener noreferrer" className="px-8 py-4 rounded-xl bg-white/20 border border-black/20 text-black font-semibold text-lg hover:bg-white/30 transition-colors flex items-center gap-3">
      ...
      <footer className="relative py-12 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-black">
              <KairoLogo className="w-5 h-5" />
            </div>
            <span className="font-semibold">CantonHub by <span className="text-orange-500">AngelHack</span></span>
          </div>
          
          <div className="flex items-center gap-6 text-sm text-gray-400">
            <a href="https://earn.stackup.dev/learn/pathways/canton-and-daml-fundamentals-328a" target="_blank" rel="noopener noreferrer" className="hover:text-orange-500 transition-colors">Learn about Canton Network</a>
